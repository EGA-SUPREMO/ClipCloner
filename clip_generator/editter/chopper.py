...
def cutLastSecondsAudio(seconds):
	...
	os.system(f"ffmpeg -y -sseof -{cutted_seconds} -i {dir_audio_clip} -c copy {dirAudioParts}temp_last_S{real_seconds}_clip_audio.mp4")
...
def chop(from_second, to_second):
	os.system(f"ffmpeg -y -ss {from_second} -to {to_second} -i {dir_stream} {dir_trimmed_stream}")
