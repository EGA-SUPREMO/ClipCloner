import unittest

from clip_generator.editter import dirs
from clip_generator.editter.info_processor import curate_results


class TestCurateResults(unittest.TestCase):
    def setUp(self) -> None:
        self.maxDiff = None

    def test_curate_results(self):
        dirs.update_phase(1)
        offset1 = [(5.14, 7.14), (29.10, 30.10), (24.38, 25.38), (9.16, 10.16), (23.1, 24.1), (11.16, 54.17)]
        expected_output1 = [(5.14, 54.17)]
        self.assertEqual(curate_results(offset1), expected_output1)

    def test_curate_results1(self):
        dirs.update_phase(1)
        offset1 = [(0.03675, 1.03675), (16.63275, 17.63275), (2.056875, 10.046125), (11.971, 12.971),
                   (11.043, 22.05075), (12.986875, 13.986875), (23.04775, 25.046125), (8.074625, 9.074625),
                   (26.043125, 27.043125), (31.563375, 32.563375), (28.04, 42.04325), (16.23925, 17.23925),
                   (43.040125, 46.037), (11.62975, 13.6275), (48.055625, 53.0495), (15.9835, 16.9835),
                   (13.68675, 14.68675), (15.892625, 16.892625000000002), (18.013625, 19.013625), (16.25625, 17.25625),
                   (16.287375, 17.287375), (7.619125, 8.619125), (16.876875, 17.876875), (13.675, 15.675)]
        expected_output1 = [(0.03675, 53.0495), (15.9835, 16.9835),
                            (13.68675, 14.68675), (15.892625, 16.892625000000002), (18.013625, 19.013625), (16.25625, 17.25625),
                            (16.287375, 17.287375), (7.619125, 8.619125), (16.876875, 17.876875), (13.675, 15.675)]
        self.assertEqual(curate_results(offset1), expected_output1)

    def test_curate_results2(self):
        dirs.update_phase(1)
        offset1 = [(6.149875, 7.149875), (29.108, 30.108), (24.38575, 25.38575), (9.168375, 10.168375),
                   (14.41475, 15.41475), (23.1, 24.1), (12.16375, 54.17175)]
        expected_output1 = [(6.149875, 54.17175)]
        self.assertEqual(curate_results(offset1), expected_output1)

    def test_curate_results3(self):
        dirs.update_phase(1)
        offset1 = [(0.07325, 6.08875), (27.153125, 28.153125), (65.282, 66.282), (8.08425, 12.07975), (57.529, 58.529),
                   (33.321125, 34.321125), (21.441625, 22.441625), (59.02475, 60.02475), (26.559375, 27.559375),
                   (24.46025, 30.4525), (31.03425, 41.043625), (26.789625, 27.789625), (42.040375, 43.040375),
                   (0.874, 1.874), (31.957375, 32.957375), (57.912375, 58.912375), (46.03425, 47.03425),
                   (27.162375, 28.162375), (90.613125, 91.613125), (49.029625, 51.028125), (26.512, 27.512),
                   (27.186375, 28.186375), (53.023625, 54.023625), (27.076, 28.076), (55.677125, 60.670875),
                   (57.324375, 58.324375), (64.601125, 66.5995), (69.46625, 70.46625), (72.996375, 75.993375),
                   (76.875125, 77.875125), (58.649875, 59.649875), (57.357375, 58.357375), (79.89375, 82.89075),
                   (27.05975, 28.05975), (84.7375, 85.7375), (58.63975, 59.63975), (86.734625, 88.733),
                   (28.673625, 29.673625), (89.730125, 93.726875)]

        expected_output1 = [(0.07325, 12.07975), (57.529, 58.529),
                            (33.321125, 34.321125), (21.441625, 30.4525), (31.03425, 54.023625), (27.076, 28.076), (55.677125, 60.670875),
                            (57.324375, 58.324375), (64.601125, 66.5995), (69.46625, 70.46625), (72.996375, 75.993375),
                            (76.875125, 82.89075),
                            (27.05975, 28.05975), (84.7375, 93.726875)]
        self.assertEqual(curate_results(offset1), expected_output1)

    def test_curate_results4(self):
        dirs.update_phase(1)
        offset1 = [(0.594125, 1.594125), (76.107875, 77.107875), (2.61425, 8.6065), (75.679125, 76.679125),
                   (23.08675, 26.083625), (26.932125, 27.932125), (144.94, 145.94), (77.40175, 78.40175),
                   (31.594125, 32.594125), (213.292375, 214.292375), (145.212625, 146.212625), (40.979375, 42.977875),
                   (52.309625, 53.309625), (145.464125, 146.464125), (60.339875, 66.33225), (77.69025, 78.69025),
                   (69.879125, 78.89), (80.4385, 83.435375), (77.413, 78.413), (84.432375, 88.427625),
                   (92.666, 94.6645), (77.729, 78.729), (100.111375, 104.10675), (105.90525, 106.90525),
                   (209.69675, 210.69675), (107.902125, 108.902125), (145.19175, 146.19175), (109.899125, 111.8975),
                   (120.712625, 122.711125), (144.945375, 145.945375), (123.73125, 125.72975), (149.378125, 152.375125),
                   (145.462, 146.462), (228.14425, 229.14425), (145.591875, 146.591875), (230.141125, 232.139625),
                   (77.40525, 78.40525), (77.004125, 78.004125), (145.48175, 147.48175)]

        expected_output1 = [(0.594125, 8.6065), (75.679125, 76.679125),
                            (23.08675, 26.083625), (26.932125, 27.932125), (144.94, 145.94), (77.40175, 78.40175),
                            (31.594125, 32.594125), (213.292375, 214.292375), (145.212625, 146.212625), (40.979375, 42.977875),
                            (52.309625, 53.309625), (145.464125, 146.464125), (60.339875, 66.33225), (77.69025, 78.69025),
                            (69.879125, 78.89), (80.4385, 88.427625),
                            (92.666, 94.6645), (77.729, 78.729), (100.111375, 104.10675), (105.90525, 111.8975),
                            (120.712625, 125.72975), (149.378125, 152.375125),
                            (145.462, 146.462), (228.14425, 232.139625),
                            (77.40525, 78.40525), (77.004125, 78.004125), (145.48175, 147.48175)]
        self.assertEqual(curate_results(offset1), expected_output1)

    def test_curate_results5(self):
        dirs.update_phase(1)
        offset1 = [(9.4985, 35.506375), (171.554125, 172.554125), (36.50325, 46.512625), (183.549125, 184.549125),
                   (41.2155, 42.2155), (48.508, 50.5065), (171.302125, 172.302125), (203.87725, 204.87725),
                   (21.678625, 22.678625), (53.500375, 55.49875), (179.64575, 180.64575), (56.518875, 60.51425),
                   (192.2095, 193.2095), (166.8885, 167.8885), (62.509625, 63.509625), (183.32275, 184.32275),
                   (179.47275, 180.47275), (172.1575, 173.1575), (171.9145, 172.9145), (183.672125, 184.672125),
                   (179.551125, 180.551125), (171.2665, 172.2665), (70.5205, 82.5035), (180.4035, 181.4035),
                   (83.5005, 84.5005), (179.845875, 180.845875), (85.520625, 96.50525), (131.904875, 132.904875),
                   (97.502125, 99.500625), (171.448625, 172.448625), (100.52075, 101.52075), (128.34225, 129.34225),
                   (102.517625, 103.517625), (126.84725, 127.84725), (179.466875, 180.466875), (105.513, 134.51625),
                   (175.392125, 176.392125), (135.513125, 138.510125), (78.535125, 79.535125), (139.507, 144.500875),
                   (128.39, 129.39), (145.521, 146.521), (179.534, 180.534), (41.71225, 42.71225),
                   (183.494125, 184.494125), (179.980875, 180.980875), (150.51325, 158.5025), (172.943875, 173.943875),
                   (159.499375, 161.521), (127.02475, 128.02474999999998), (162.518, 184.508875),
                   (186.290625, 193.304625), (195.453, 196.453), (199.368125, 211.35275)]

        expected_output1 = [(9.4985, 184.508875), (186.290625, 193.304625), (195.453, 196.453), (199.368125, 211.35275)]
        self.assertEqual(curate_results(offset1), expected_output1)

    def test_curate_results6(self):
        dirs.update_phase(1)
        offset1 = [(10.017, 13.037125), (17.241625, 18.241625), (143.850875, 144.850875), (17.396, 18.396),
                   (17.16075, 18.16075), (17.029375, 23.02175), (17.36425, 18.36425), (99.996875, 100.996875),
                   (25.017125, 26.017125), (17.718625, 18.718625), (51.2205, 53.219), (17.700125, 18.700125),
                   (144.101125, 145.101125), (55.214375, 62.205125), (77.77025, 78.77025), (17.55475, 18.55475),
                   (144.271375, 145.271375), (17.510875, 18.510875), (81.78725, 84.78425), (17.351625, 18.351625),
                   (17.5815, 18.5815), (86.779625, 96.788875), (17.966625, 18.966625), (97.785875, 99.784375),
                   (107.699375, 118.684), (17.605875, 18.605875), (167.691875, 168.691875), (120.702625, 122.701125),
                   (17.5365, 18.5365), (123.698, 127.693375), (40.597125, 41.597125), (17.06525, 18.06525),
                   (129.68875, 130.68875), (17.194875, 18.194875), (131.685625, 135.70425), (17.20475, 18.20475),
                   (136.701125, 162.685875), (167.897125, 168.897125), (163.706, 171.69675)]

        expected_output1 = [(10.017, 26.017125), (17.718625, 18.718625), (51.2205, 62.205125), (77.77025, 99.784375),
                            (107.699375, 171.69675)]
        self.assertEqual(curate_results(offset1), expected_output1)

    def test_curate_results_artificial_offset(self):
        dirs.update_phase(1)
        offset1 = [(5, 10), (20, 22), (12, 15), (22, 23), (16, 18), (48, 50), (20, 24), (26, 28)]
        expected_output1 = [(5, 24), (26, 28)]
        self.assertEqual(curate_results(offset1), expected_output1)

    def test_curate_results_artificial_offset_do_not_merge_if_there_is_not_an_appropriate_range_in_between(self):
        dirs.update_phase(1)
        offset1 = [(8, 10), (19, 21), (11, 13), (22, 23)]
        expected_output1 = [(8, 10), (19, 21), (11, 13), (22, 23)]
        self.assertEqual(curate_results(offset1), expected_output1)

    def test_curate_results_artificial_offset2(self):
        dirs.update_phase(1)
        offset1 = [(8, 9), (5, 10), (20, 21), (11, 15), (22, 23)]
        expected_output1 = [(8, 9), (5, 15), (22, 23)]
        self.assertEqual(curate_results(offset1), expected_output1)

    def test_curate_results_artificial_offset3(self):
        dirs.update_phase(1)
        offset1 = [(0, 9), (10, 11), (20, 41), (11, 15), (22, 23)]
        expected_output1 = [(0, 9), (10, 11), (20, 41), (11, 15), (22, 23)]
        self.assertEqual(curate_results(offset1), expected_output1)

    def test_curate_results_artificial_offset4(self):
        dirs.update_phase(1)
        offset1 = [(8, 10), (19, 21), (11, 13), (23, 24)]
        expected_output1 = [(8, 10), (19, 24)]
        self.assertEqual(curate_results(offset1), expected_output1)

    def test_curate_results_artificial_offset_do_not_merge_if_there_is_coincidence_in_between(self):
        dirs.update_phase(1)
        offset1 = [(0, 9), (10, 11), (20, 41), (22, 23), (22, 23)]
        expected_output1 = [(0, 9), (10, 11), (20, 41), (22, 23), (22, 23)]
        offset2 = [(4, 13), (10, 11), (20, 41), (35, 36), (31, 32)]
        expected_output2 = [(4, 13), (10, 11), (20, 41), (35, 36), (31, 32)]
        self.assertEqual(curate_results(offset1), expected_output1)
        self.assertEqual(curate_results(offset2), expected_output2)

    def test_curate_results_artificial_offset_merge_when_appropriate(self):
        dirs.update_phase(1)

        offset3 = [(4, 6), (10, 12), (40, 42), (9, 11), (12, 14)]
        expected_output3 = [(4, 14)]
        offset4 = [(4, 6), (10, 12), (40, 42), (40, 42), (9, 11), (14, 17)]
        expected_output4 = [(4, 17)]
        offset5 = [(4, 6), (10, 12), (10, 12), (40, 42), (40, 42), (9, 11), (16, 18)]
        expected_output5 = [(4, 6), (10, 12), (10, 12), (40, 42), (40, 42), (9, 11), (16, 18)]
        self.assertEqual(curate_results(offset3), expected_output3)
        self.assertEqual(curate_results(offset4), expected_output4)
        self.assertEqual(curate_results(offset5), expected_output5)

    def test_curate_results_artificial_offset_do_not_merge_if_there_is_coincidence_in_between1(self):
        dirs.update_phase(1)
        offset1 = [(0, 9), (10, 11), (20, 41), (23, 24), (21, 22)]
        expected_output1 = [(0, 9), (10, 11), (20, 41), (23, 24), (21, 22)]
        self.assertEqual(curate_results(offset1), expected_output1)

    def test_curate_results_when_there_is_two_separate_consecutive_numbers(self):
        dirs.update_phase(1)
        offset1 = [(5, 10), (20, 21), (11, 13), (22, 23)]
        expected_output1 = [(5, 13), (22, 23)]
        self.assertEqual(curate_results(offset1), expected_output1)

    def test_curate_results_when_there_is_two_consecutive_numbers_wrong(self):
        dirs.update_phase(1)
        offset1 = [(8, 10), (19, 21), (12, 14), (22, 23)]
        expected_output1 = [(8, 14), (22, 23)]
        self.assertEqual(curate_results(offset1), expected_output1)

    def test_curate_results_when_there_is_two_consecutive_numbers_wrong1(self):
        dirs.update_phase(1)
        offset1 = [(9, 10), (19, 21), (11, 13), (23, 24)]
        expected_output1 = [(9, 10), (19, 24)]
        self.assertEqual(curate_results(offset1), expected_output1)

    def test_curate_results_do_not_remove_wrong_match_if_there_isnt_enough_items_in_between(self):
        dirs.update_phase(1)
        offset1 = [(2.372125, 5.39225), (7.124125, 11.1195), (13.117875, 17.11325), (26.895125, 28.8935),
                   (30.758625, 37.772625), (40.43775, 44.433125), (49.664875, 55.65725), (69.781125, 70.781125),
                   (50.30375, 51.30375), (53.418, 54.418), (71.660125, 72.660125), (42.76025, 43.76025),
                   (65.521125, 74.508875), (69.247625, 70.247625), (68.871625, 69.871625), (34.316625, 36.316625)]
        expected_output1 = [(2.372125, 5.39225), (7.124125, 11.1195), (13.117875, 17.11325), (26.895125, 28.8935),
                            (30.758625, 37.772625), (40.43775, 44.433125), (49.664875, 55.65725), (69.781125, 70.781125),
                            (50.30375, 51.30375), (53.418, 54.418), (71.660125, 72.660125), (42.76025, 43.76025),
                            (65.521125, 74.508875), (69.247625, 70.247625), (68.871625, 69.871625), (34.316625, 36.316625)]
        self.assertEqual(curate_results(offset1), expected_output1)

    def test_curate_results_ignore_consecutive_numbers_if_they_can_be_merged(self):
        dirs.update_phase(1)

        offset2 = [(0.915125, 3.93525), (12.295875, 13.295875), (48.928, 49.928), (5.930625, 19.933875), (11.7425, 12.7425), (66.926875, 67.926875), (21.92925, 44.9185), (48.462125, 49.462125), (48.97725, 49.97725), (46.937125, 63.93725)]
        expected_output2 = [(0.915125, 63.93725)]
        self.assertEqual(curate_results(offset2), expected_output2)

    def test_curate_results_do_not_merge_if_there_is_not_an_appropriate_items_between_numbers(self):
        dirs.update_phase(1)

        offset3 = [(5.14, 7.14), (9.16, 10.16), (11.16, 54.17)]
        expected_output3 = [(5.14, 7.14), (9.16, 10.16), (11.16, 54.17)]
        self.assertEqual(curate_results(offset3), expected_output3)


if __name__ == '__main__':
    unittest.main()
